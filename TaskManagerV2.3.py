...
for i in range (data):
    dis = Label(root, text=c).pack()
    dis = Label(root, text=a).pack()
    dis = Label(root, text=b).pack()

question_num = len(data)  # check how many questions are already in the quiz 
for i in range(how_many):
    new = dict()
    new["taskname"] = input("Enter the taskname: ").capitalize() #This will be replaced with an entry box
    new["discription"] = input("Enter a discription") # this will be replaced with an entry box
    new["Duedate"] = input("Enter the Due date") # this will be replaced with the entered duye date

    c_key = "Task" + str(question_num+i+1)
    data[c_key] = new
